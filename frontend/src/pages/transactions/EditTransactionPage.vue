<template><section class="page"><van-nav-bar title="编辑交易" left-arrow @click-left="router.back()" /><div v-if="loadingInitial" class="state-card"><van-loading /></div><van-empty v-else-if="error && !transaction" image="error" :description="error"><van-button @click="load">重试</van-button></van-empty><TransactionForm v-else :initial="transaction" :loading="saving" @submit="save" /></section></template>
