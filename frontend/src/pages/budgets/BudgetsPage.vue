...
function percentage(value:string){
  ...
  const scaled=(BigInt(`${integer}${fraction}`)*100n).toString().padStart(fraction.length+1,'0')
  if(fraction.length===0)return scaled
  const whole=scaled.slice(0,-fraction.length)||'0'
  const decimals=scaled.slice(-fraction.length).replace(/0+$/,'')
