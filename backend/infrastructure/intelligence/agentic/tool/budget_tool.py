...
def budget_tool(start_date: str, end_date: str) -> str:
    """
    查询指定日期范围内的预算执行情况。
    ...
    try:
        ...
        try:
            ...
            # 同步查询预算
            budgets = _find_budgets_sync(db_session, start, end)
            ...
            # 格式化输出
            return _format_budgets(budgets)
        finally:
            db_session.close()
...
def _find_budgets_sync(session, start_date: date, end_date: date) -> List[Budget]:
    """同步方式查询预算"""
...
def _format_budgets(budgets: List[Budget]) -> str:
    """格式化预算详情"""
    lines = []
