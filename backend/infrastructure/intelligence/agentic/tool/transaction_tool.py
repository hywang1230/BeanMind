...
class TransactionTool(Tool):
    """
    交易工具，用于查询交易信息并格式化为 beancount 格式
    """
    
    def execute(self, start_date: str, end_date: str) -> str:
        """
        执行工具，查询交易信息并格式化为 beancount 格式文本
        ...
        try:
            ...
            try:
                ...
                # 格式化为 beancount 文本
                return self._format_transactions_to_beancount(transactions)
            finally:
                db_session.close()
    ...
    def _format_transactions_to_beancount(self, transactions: List[Transaction]) -> str:
        """
